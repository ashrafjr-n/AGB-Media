...
| `components/Founder/Founder.jsx` | The founder's page-within-a-section — an ordinary section after About, whose **four areas** reveal on the shared ladder (intro, career line, photo strip, works strip). Its ground is a **still image** (`f.png`) filling **one screen** under a frosted pane of the **light** glass pair — `--section-glass-fill-light` / `--section-glass-blur-light` — both at a negative `z-index` so the in-flow copy stays on top. It was `story.webm` under the strong pair; it holds **no video** and no longer claims a playback slot — see §7. Runs **two** LogoLoops, deliberately opposed in direction. The portrait sits in a `.portrait-frame` that exists only to clip it — §7. |
...
| `src/assets/images/f.png` | **3.3 MB**, 2122×1186 — the Founder's ground: a studio still, lit from the upper right. Vite-imported, so it is hashed and bundled. `loading="lazy"` is doing real work here: the section is three viewports down, and this is the largest asset on the page after the two videos. **It is a PNG of a photographic image** — the one asset on the site that would obviously benefit from being re-encoded (a WebP/AVIF at this size is typically a fifth of it). | in use (Founder ground) |
...
**Two `<video>` elements exist; at most one is ever running.** `hero.webm` in Hero, and `story.webm` in the Story circle (`CssLens`). There were three until the Founder's ground became a still image (`f.png`) — that section carried a second `<video>` pointing at the same `story.webm`, which is why this rule needed arbitrating at all.
