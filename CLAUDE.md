...
**THE PIN IS GATED TO `min-width: 48rem` AND `min-height: 45rem`, and the gate is structural rather than cosmetic.** A box pinned to the block start shows its own first screen and nothing below it, so pinning a section taller than the viewport makes its lower half permanently unreachable — on a phone that is four of the six reasons. The two conditions are the envelope WhyAgb was designed and verified against: `48rem` is where its grid becomes 2 × 3 (below it, six cells in one column run two or three screens and no height helps), and `45rem` is the 720px of the 1280×720 window its one-screen budget was measured at. Outside that envelope the two are ordinary siblings in flow, exactly as before. **Anything that makes WhyAgb taller has to be checked against this gate, not just against the section's own look.**

**Team's rounded block-start corners are part of the mechanic, not decoration.** `--team-radius` is `clamp(1.75rem, 4.5vw, 4rem)`, sized for the state the section spends most of its time in — filling the window — rather than for the moment it arrives; a radius tuned on a small box reads as square at 1440px. `overflow: hidden` is what gives the radius teeth, clipping the absolutely positioned full-bleed video and its pane to the curve while leaving the video full-bleed. It clips nothing else: the footer's negative margins reach exactly the border box and `.button-featured`'s glow ring is already clipped by its own variant. **The `overflow` is safe here specifically because Team is the sticky element's *sibling*, not its ancestor** — `overflow` on the wrong element is the one thing that silently breaks a sticky pairing. A `--glass-border` hairline runs the block-start edge because both surfaces meeting there are near-black and the curve would otherwise arrive invisible through the clip's dark stretches; `box-sizing: border-box` means it costs no height.
