...
| `components/WhyAgb/WhyAgb.jsx` | The closing section — "Why AGB Media", six reasons in a **2 × 3 grid** on one screen, over a drifting field of outlined wordmarks. Each cell is an oversized gold numeral (8%) sitting *behind* its title, with one hairline down the grid's centre and no other rule, border or fill. Copy lives in a local `reasons` array — `title` + `description` only, no per-row label. The field is three rows of `-webkit-text-stroke` text drifting on pure CSS keyframes, middle row opposed — see §2 and §7. |
