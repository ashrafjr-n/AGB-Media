...
**Thirteen design tokens in `variables.css` are unreferenced, and are deliberately kept:** `--color-gold-deep`, `--color-border`, `--color-border-subtle`, `--color-overlay`, `--color-glass`, `--glass-sheen`, `--glass-sheen-warm`, `--header-blur`, `--text-lg`, `--text-xl`, `--text-2xl`, `--tracking-widest`, `--radius-sm`. **`--ease-in-out` has come off this list** — the Team section's circle hover and both headers' nav-link hover use it, the first call sites it has ever had, which is exactly what it was kept for. `--duration-slow` briefly joined it there and is unreferenced again: 600ms is the right feel for something settling and the wrong one for a pointer state, so that hover came back to `--duration-base`. They are listed here rather than deleted, per this section's own rule. Most are **rungs of a scale** — deleting the unused steps of a type, radius or easing ramp leaves a ramp with holes in it, which is worse than an unused custom property that costs nothing at runtime. The palette entries are the same argument: they are what a new surface is built from. `--glass-sheen` is the newest arrival, orphaned when the hero ticker moved to `FluidBar`; `--color-glass` and `--header-blur` were orphaned when the header pill went to white glass.
