...
| `components/WhyAgb/WhyAgb.jsx` | The closing section — "Why AGB Media", six reasons as **numbered editorial rows** (01–06) divided by hairlines, not a grid of cards. Flat `--color-black`, gold numerals at 45%, the shared reveal ladder. Copy lives in a local `reasons` array. |
...
| `src/assets/videos/story.webm` | **4.7 MB.** Vite-imported and bundled, and it wants the opposite treatment to the hero's. **Imported twice** — by `About` for the Story circle and by `Founder` for its background — which emits **one** hashed asset and costs one download; the two `<video>` elements are two decoders sharing it, and never both running (§7). `CssLens` sets `preload="metadata"` so it fetches a first frame rather than 4.7 MB while the hero needs the bandwidth; `Founder` starts at `preload="none"` and warms up 400px before it is reached. | in use (Story circle + Founder ground) |
...
- **The observation carries 400px of margin either side** (`GLASS_REACH_MARGIN`, the same lead the Founder's preload uses). Re-creating a compositor layer can cost a frame, and that frame must not be the one the glass first samples on.
...
**Three `<video>` elements exist; at most one is ever running.** `hero.webm` in Hero, and `story.webm` twice — once in the Story circle (`CssLens`) and once as the Founder's background. The second and third are the *same file*, so Vite emits one hashed asset and the browser fetches it once; the duplication is two decoders, not two downloads.
...
- **Story vs. Founder is arbitrated**, in `hooks/useExclusiveVideo.js`. It has to be: the sections are adjacent and each is at least 100vh, so at the boundary a fifth of both is on screen and both want to play. Each registers a claim (a `videoRef`, a priority, and a boolean intent); one module-scoped `resolve()` recomputes the whole state from all claims on every change, pauses every loser in a pass of its own, and only then plays the winner.

`PLAYBACK_PRIORITY` is document order — **Story wins the overlap** — and the tie-break is argued rather than incidental: the Story circle shows its footage *sharp*, so a frozen frame there is obvious, while the Founder's copy is under `--section-glass-blur` and a 0.75 scrim where a still frame and a moving one are very hard to tell apart. Freeze the one nobody can see freezing. The visible consequence: scrolling down, the Founder's background is still until the Story section has mostly left.
...
`CssLens` renders its `<video>` **without `autoPlay`**, and so does Founder — putting `autoPlay` back on either reintroduces a decode nothing asked for. The Founder's also starts at **`preload="none"`**, promoted to `preload = 'auto'` + `load()` by a second `useInView` with a 400px root margin; left at the default it would start buffering during first paint, four viewports below the fold, against the hero's 3.7MB.
