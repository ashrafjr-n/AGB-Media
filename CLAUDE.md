...
**Audited and clean in the same pass:** every class in every home-page CSS Module is still referenced from JSX or `composes`d; no unused imports; both new custom properties have exactly one consumer each. **The unreferenced-token list in this section is down to nine** — `--color-gold-deep`, `--color-border`, `--color-border-subtle`, `--color-overlay`, `--glass-sheen-warm`, `--text-lg`, `--header-blur`, `--radius-sm`, `--duration-slow` — because `--glass-sheen` and `--tracking-widest` have both since gained call sites (the latter in Footer's phone rule). Still deliberately kept, for the reasons already given below.
