...
**Two guards, not one, and they answer different questions.** `once: true` stops a ladder replaying on every scroll past — but only while the page stays mounted, and this page remounts constantly: every nav destination except `/` is wired ahead of its route, so following one unmounts `HomePage` entirely and coming back rebuilds it. Every section then faded up from nothing again, most visibly on About's "Discover Our Story" button, because that is the link people actually click and therefore the element they return to.

So the hook takes a **scope** — `useSectionReveal('about')`, `'founder'`, `'why-agb'`, `'team'` — and records it in a module-scoped `Set` when that section's reveal first completes. On any later mount every step returns `{}` and the section renders in its final state. It is HeroHeader's `entrancePlayed` pattern with one difference that is the whole design: **it is keyed per section rather than a single flag.** One global boolean would latch the moment any section revealed, so a reader who followed a link out of the Story section would find the Founder, WhyAgb and Team rendered flat on their return — sections they had never seen animate. Latching is **on completion, not on mount**, for the same StrictMode reason HeroHeader documents. Calling the hook with no scope keeps the old per-mount behaviour.
...
| Nav-link hover | colour only, `--color-text-muted` → `--color-text-bright` | identical — same tokens, same duration and curve |
...
**Thirteen design tokens in `variables.css` are unreferenced, and are deliberately kept:** `--color-gold-deep`, `--color-border`, `--color-border-subtle`, `--color-overlay`, `--color-glass`, `--glass-sheen`, `--glass-sheen-warm`, `--header-blur`, `--text-lg`, `--text-xl`, `--text-2xl`, `--tracking-widest`, `--radius-sm`. **`--duration-slow` and `--ease-in-out` have come off this list** — the Team section's circle hover is the first call site either has ever had, which is exactly what they were kept for. They are listed here rather than deleted, per this section's own rule. Most are **rungs of a scale** — deleting the unused steps of a type, radius or easing ramp leaves a ramp with holes in it, which is worse than an unused custom property that costs nothing at runtime. The palette entries are the same argument: they are what a new surface is built from. `--glass-sheen` is the newest arrival, orphaned when the hero ticker moved to `FluidBar`; `--color-glass` and `--header-blur` were orphaned when the header pill went to white glass.
