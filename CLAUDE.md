...
`HomePage` renders `<Header />`, then `<main>` with **two sticky stages and one ordinary section between them**: a `.stage` wrapper holding `<HeroBackdrop />`, `<Hero />` and `<About />`; then `<Founder />`; then a `.closing-stage` wrapper holding `<WhyAgb />` and `<Team />`. **`<Footer />` renders inside `Team`**, as that section's last child, so there is no page-level element after it — see §7. There is no placeholder section any more — `WhyAgb` took that slot, and `Team` closes the page below it. `HomePage.module.css` went with it and has since come back, for the stage — see §7.
...
| `pages/HomePage.jsx` | The home page. Owns **both stages**: `.stage` (`HeroBackdrop`, `Hero`, `About` — scopes the sticky footage to those two sections) and `.closing-stage` (`WhyAgb`, `Team` — scopes WhyAgb's pin to the pair). Both are argued in §7. Holds **one boolean**, reported by `About` and consumed by a component that knows nothing of it: whether the Story section is half on screen (→ `Header`'s `visible`, §4). It held a second until the backdrop became sticky and made the question unnecessary. |
...
| `components/shared/Button.module.css` | **The** button definition — base + **three** variants, two glass and one solid. A module with no `.jsx` sibling, on purpose. |
| `components/shared/SectionGlass.module.css` | **The** section pane — the light glass pair and nothing else, `composes`d by both `Founder`'s `.glass` and `WhyAgb`'s. Also no `.jsx` sibling: what is shared is a surface, and each call site needs its own edge, height and depth. It exists because those two sections meet at a seam through one image, so their panes must composite identically; one rule means there is nothing to drift. **No gradient, blend mode or element `opacity` may be added to it** — each would make the pane's effect depend on position or on what is beneath, which is the mismatch it prevents. |
...
| `components/Founder/Founder.jsx` | The founder's page-within-a-section — an ordinary section after About, whose **three areas** reveal on the shared ladder (intro, career timeline, CTA — both scrolling strips that used to sit between them are gone, 2026-08-02). Its ground is a **still image** (`founder.webp`) filling **one screen** under a frosted pane of the **light** glass pair — `--section-glass-fill-light` / `--section-glass-blur-light` — both at a negative `z-index` so the in-flow copy stays on top, and both pinned to the section's **block end** because that edge is a seam — §7. It was `story.webm` under the strong pair; it holds **no video** and no longer claims a playback slot — see §7. **Runs no LogoLoop any more** — see §7. The portrait sits in a plain `.portrait-frame` that exists only to clip it — no ring, no halo; one lived here briefly and was removed for bleeding past the section's own top edge, see §7. `.intro` is one text column (`.profile`: eyebrow, name, bio, the four metadata facts as icon cards on `--color-glass`) against the portrait alone on the right. The career line is a five-node icon timeline (`.timeline`) with icons inside every node and one continuous connecting rail at every width, horizontal on a row layout and vertical on a phone. A `.button-solid` CTA — "Discover More", to a placeholder route — closes the section below the timeline — see §7. |
| `components/WhyAgb/WhyAgb.jsx` | "Why AGB Media", six reasons in a **2 × 3 grid** on one screen. **It is the page's second `position: sticky` element** — pinned inside `.closing-stage` while Team scrolls up over it, above `64rem × 45rem` only (§7). Each cell is a gold numeral labelling its title, with one hairline down the grid's centre and no other rule, border or fill. Copy lives in a local `reasons` array — `title` + `description` only, no per-row label. Its ground is `why.webp`, the second half of the Founder's image, under the same light glass pair pinned to the section's **block start** — the two grounds are one picture cut across the section boundary, §7. It was four radial gradients, a block-axis mask and a local grain layer, and before that a **drifting field of outlined "AGB MEDIA" wordmarks** — three rows of `-webkit-text-stroke` text on infinite CSS keyframes, the middle one carrying a pointer-follow loop. All of it is deleted: markup, keyframes, the `--why-field-stroke` dial, the RAF loop and its listeners. **Nothing in this component animates outside the shared reveal ladder**, so it no longer calls `useReducedMotion` at all. |
| `components/Team/Team.jsx` | The closing section — "The People Behind AGB", the CEO in a large circle over a row of **four placeholder seats**, on `story.webm` running full bleed under the lightest *video* pane on the site. **It arrives over a pinned WhyAgb**, so its block-start corners are rounded at `--team-radius` (a generous clamp, sized to still read once the section fills the window) with `overflow: hidden` clipping the full-bleed video to them and a `--glass-border` hairline making the curve legible where two near-black surfaces meet — §7. Registers the page's **second** `useExclusiveVideo` claim (`team: 1`) and gates on its own box, so it plays only in view. The four seats are a local `seats` array of `{ name, title, image }` with `image: null` — filling them in is a data edit, not a markup one, and an unfilled seat draws `HiOutlineUser` from react-icons rather than a fabricated avatar (§5). |
| `components/Footer/Footer.jsx` | The page's closing bar (~71px at 900px tall), rendered **inside the Team section** as its last flex child rather than after it — "© All Rights Reserved 2026" on the inline start, a row of small links on the inline end, over `.pane` from the shared glass. It paints no background of its own and breaks out of Team's inline and block-end padding with negative margins, so it is full-bleed and flush to the section's bottom edge; those two values are copies of Team's and must move together. See §7. The link row is a **placeholder** built from `navLinks` minus the featured entry; an entry may be `{ label, to }` for a route or `{ label, href }` for anything external, and the render already branches on which, so swapping in socials is a data edit. |
...
| `src/assets/abdullah/*.jpg` | 1280×720 originals. Only `abdullah.jpg` is imported (the portrait, drawn at ~440×350). The six numbered stills are **not** — nothing imports them, so Vite does not bundle them. **Their `strip/` cuts are gone** (368 KB, 640×360) — deleted 2026-08-02 with the photo strip that was their only caller; these six stay, still source only, still there to re-cut from if a strip returns. | source only |
...
- **Exactly one surface flows, and it is the ticker** — genuine fluid motion under a displacement filter. There was briefly a second, WhyAgb's drifting field of outlined wordmarks, admitted on the argument that it was *not the same material*: a texture painted **on** the ground rather than a surface, at a stroke too faint to carry a material reading. It has since been deleted along with the rest of that section's built background, so the exception is spent and the rule is back to its simple form. **A second moving surface needs its own argument, and "WhyAgb had one once" is not it.**
...
| `--about-ground` | `#0B0704` | **The `/about` page's ground, and the one thing on the site that is not `--color-black`.** The accent's own hue taken almost to black (hue ~26°, the logo's warm lean), a few points lighter than the site ground — which is most of what makes it register as brown, since at this luminance hue alone is nearly invisible. It was `#130C07` and came down by about half its luminance; the channel *ratio* is what carries the hue and is unchanged. Warmth comes from `--about-sheen`, not from lifting this. |
| `--about-vignette` | `rgba(4,2,1,0.85)` | The `/about` vignette: that ground's **own** tone deepened, never raw black. Pure black over a warm ground greys the corners rather than deepening them, which is the flat edge a vignette exists to avoid. **It moves whenever `--about-ground` moves** — a vignette is a relationship, and at the old value against the new ground it would have inverted into a corner glow. |
...
| --- | --- | --- |
| `.button .button-featured` | Glass over a **frozen turbulence texture**, `--radius-md` corners, `--color-text` label, faint rim; a gold light travels the inside of the rim and pauses on hover | Hero header — the nav's own "Contact" entry — **and Team**, below the team row |
...
**`.button-featured` WAS the site's one high-emphasis button, and there are now two** — the hero header's Contact entry and the Team section's, which is the same base plus the same variant pointing at the same `/contact` route from `navLinks`. That was a deliberate addition, not a drift: the page needed a way out at the bottom and forking the variant to get one would have been worse. What it costs is the rule this paragraph used to state, so state the new one instead: **these two are the complete set.** They are a screen apart at opposite ends of the page and never share a viewport, which is what keeps the emphasis meaningful; a third, or a second one visible at the same time as either, does cancel it. It was a solid gold fill with a near-black label before it became glass.

**Both call sites gate the rim light on an ancestor's `data-offscreen`** — `.hero` for the first, the Team `<section>` for the second. That attribute is why a second call site needed no change to `Button.module.css` at all, and a third would have to do the same: the animation is a full re-raster per frame (§7) and must never run for a button nobody can see.
...
src/
  components/
    ...
    Team/
      ...
      Team.module.css
    Footer/
      Footer.jsx            # the compact closing bar — rendered INSIDE Team
      Footer.module.css
...
`once` works: it stops the observer re-firing. What replayed the reveal was the **`initial` staying attached for the element's whole life**. An element with `whileInView` and no `animate` resolves to its `initial` whenever the viewport state is re-evaluated while it is off screen — so a section that never re-renders keeps its finished state forever, and one that re-renders on scroll silently snaps back to `opacity: 0` the moment it leaves the screen and reveals again on the way back. That was exactly the split: WhyAgb re-renders never and held at opacity 1; About re-renders constantly (two `useInView` observers plus a `useScrollPosition` selector) and its "Discover Our Story" button measured `opacity: 0; transform: translateY(12px)` while off screen, on the same DOM node. Same hook, same props, opposite behaviour, decided by whether the section happens to watch its own scroll position.

So the hook takes a **scope**, keys each rung as `scope:step` in a module-scoped `Set`, and once a rung has played its step returns `SETTLED` — `initial: false` with a static `animate` at the end state. No `whileInView`, no `viewport`, no observer, and no `initial` for a re-render to fall back to. Three things about it are load-bearing:

- **`SETTLED`, not `{}`.** Returning an empty object is the obvious move and leaves the inline `opacity: 0` that `initial` wrote with nothing to clear it — the element plays its reveal and then vanishes. Measured.
- **Keyed per rung, not per section and not globally.** A single flag latches the moment any reveal completes, so a fast scroller finds whole sections rendered flat they never saw animate; per section has the same bug one level down, since a tall section's first rung can finish long before its last has entered view. A component running two independent ladders needs two scopes — WhyAgb passes `'why-agb'` and `'why-agb-grid'`, because its grid restarts the stagger at 0 and would otherwise collide with the heading's rungs.
- **Recording is passive — a latch must NOT force a re-render.** Notifying consumers on each latch made the swap immediate, which re-rendered the section while later rungs were still animating and flashed them to nothing for ~240ms. Measured. Consumers pick the key up on their next natural render, which for a re-rendering section is the first render after the animation finished — exactly when the replay would otherwise arm — and a section that never re-renders cannot replay anyway.

Latching is **on completion, not on mount**, for the same StrictMode reason HeroHeader documents. Calling the hook with no scope keeps the old per-mount, per-intersection behaviour.

The hook also carries the `prefers-reduced-motion` branch (it returns `{}` per step, so elements render in their final state with no `initial`), which is why no call site checks the preference for a reveal.
...
| Nav-link hover | colour only, `--color-text-muted` → `--color-text-bright`, at `--duration-fast` + `--ease-in-out` | identical — same tokens, same duration and curve |
...
## 7. Unused, and stale — as of 2026-08-02

**THE CLOSING PIN IS DECIDED IN ONE PLACE NOW — 2026-08-02.** The pin/tuck envelopes were the clearest case of breakpoint-tuning debt on the site: the same four `@media` conditions were written out **twice**, once in `WhyAgb.module.css` to switch `position: sticky` on and again in `Team.module.css` to switch `--team-tuck` on — eight blocks across two files for four decisions, with a comment in each file instructing the next reader to keep the copies identical by hand and naming the breakage if they drifted. All eight are gone. The four envelopes now live once, on **`.closing-stage` in `pages/HomePage.module.css`**, the only element that is an ancestor of both sections, and set two inherited custom properties:

- **`--closing-pin-inset`** → WhyAgb's `inset-block-start`. That section is **unconditionally `position: sticky`** now; outside every envelope the property is unset and it falls back to `auto`. **A sticky box with no inset is exactly a relatively positioned one** — same geometry at every scroll offset, same height, same stacking context, same document height. Verified by measurement against the old `position: relative`, not assumed. Nothing toggles `position` any more; the envelopes only decide whether there is an edge to stick to.
- **`--closing-pinned`** → a 0/1 flag Team multiplies its own tuck by. It is a second property rather than a reuse of the first because the tuck needs arithmetic and the inset needs the keyword `auto`, and no `calc()` produces a keyword. Team keeps the arithmetic locally (`calc(var(--closing-pinned, 0) * (var(--team-radius) + 0.25rem))`) because **a custom property's `var()`s resolve on the element that declares it** — a `--team-tuck` written on `.closing-stage` would look for `--team-radius` there and find nothing.

The cross-file "keep these identical" invariant is therefore structural rather than remembered, and the measurement tables behind each number moved to `.closing-stage` with the envelopes so a gate and its evidence live together. **`.closing-stage`'s long "it is deliberately bare" list still holds** — custom properties are the one exception, since they set inherited values and give the box no position, stacking context, backdrop root or containing block.

**What this does NOT fix, stated so nobody re-derives it as a bug.** These are still four discrete envelopes. "Does the content fit one screen" is a question about *rendered height*, which CSS cannot ask, so arbitrary in-between viewport sizes are not all covered — the gates are deliberately conservative, and the failure mode at an untuned size is a section that scrolls normally when it could have pinned, never one that pins when it cannot fit. Closing the remaining band means measuring WhyAgb at runtime and toggling a flag — a `ResizeObserver` and a threshold, precisely the machinery `HeroBackdrop`'s rewrite deleted — and was judged not worth buying back for a case only a resizing browser window reaches.

**Removed in the same pass: WhyAgb's `@media (min-width: 80rem) { .row }` block.** It declared exactly the `repeat(3, minmax(0, 1fr))` the `64rem` block already had — a no-op kept on the argument that the pin's own media query sat a few lines below in that file and keyed off `80rem` by name, so holding the selector kept that reference pointing at a breakpoint that visibly existed. Moving the pin out took the last reader of that reference with it. `64rem` is where three-up starts and there is no separate widest tier.

**Audited and clean in the same pass:** every class in every home-page CSS Module is still referenced from JSX or `composes`d; no unused imports; both new custom properties have exactly one consumer each. **The unreferenced-token list in this section is down to eight** — `--color-gold-deep`, `--color-border`, `--color-border-subtle`, `--glass-sheen-warm`, `--text-lg`, `--header-blur`, `--radius-sm`, `--duration-slow` — because `--glass-sheen` and `--tracking-widest` have both since gained call sites (the latter in Footer's phone rule), and **`--color-overlay` has come off it too**: the 404 page's vignette is the "next dark translucent surface" it was kept for, and it is exactly the case the "never darken with raw black" rule exists to serve. Still deliberately kept, for the reasons already given below.

**Flagged, not acted on:** `src/assets/nael/nael-profile/` holds **eighteen unimported images, ~1.2 MB**, of which only `nael-profile.webp` is imported (by `NaelPage.jsx`). They are source-only in the same sense the `abdullah/00X.jpg` originals are, but unlike those they are not documented anywhere as deliberate. They belong to `/team/nael-al-jarabah` rather than the home page, so they were outside this pass's scope and were left alone — worth a decision next time that route is touched. Nothing bundles them, so they cost install footprint only.

**The Founder section's two scrolling strips are gone — 2026-08-02.** The bordered photo filmstrip (Abdullah's work stills) and the separate right-to-left text-credits strip both sat between the intro and the career timeline; both are deleted outright, not hidden or emptied. That freed the section's middle, so `.founder` is now a `justify-content: center` column of three areas — intro, timeline, a new CTA — with a single `--founder-gap` between them and the reveal ladder renumbered `revealAt(0..2)` to match. Consequences, so nobody re-derives them as bugs:

- **`shared/LogoLoop.jsx` and `shared/LogoLoop.module.css` are deleted, not merely unused.** The two strips were its only call sites anywhere in the codebase (confirmed by grep across every `.jsx` file); with both gone the component had zero remaining callers. This supersedes the 2026-08-01 entry below that listed its `fadeOut`/`fadeOutColor` props as deliberately-kept API breadth — that reasoning held while the component had a caller and stops holding once it has none. A generic strip primitive is easy to rebuild from git history if a future section needs one; a permanently unmounted one is dead weight, per this codebase's own precedent for `Grainient.jsx` and `FluidLens.jsx`.
- **`src/assets/abdullah/strip/*.jpg` (the six 640×360 cuts) are deleted with it** — `LogoLoop` was their only importer. The six 1280×720 originals one level up in `src/assets/abdullah/` are untouched; they were already source-only before this and stay source-only now, still there to re-cut from if a strip ever returns.
- **The bio paragraph widened** from `42ch` to `84ch` now that the strips no longer share the row — the extra horizontal room is what lets it read as four lines instead of wrapping tighter. Arithmetic-derived from the character measure, not confirmed against a rendered browser (the dev server cannot be run here) — worth a first look if the section is ever screenshotted.
- **The portrait shifted down** slightly (`.portrait-frame`'s own `margin-block-start`) rather than the whole `.intro` row's alignment changing, so the profile column's baseline is untouched.
- **A third button variant, `.button-solid`, exists now** — a flat gold fill, `--color-black` label, no glass, no pseudo-element material — for a new "Discover More" CTA closing the section below the timeline. It is reasoned at `Button.module.css` itself: the site's one-high-emphasis-button convention (§2) targets *raised/glass* surfaces specifically, and this variant never shares a viewport with `.button-featured`'s two call sites, a screen away at either end of the page.

A third pass ran on 2026-08-01, deliberately light. What it found:

- **`src/assets/images/.gitkeep` removed.** The 2026-07-31 pass kept it because that directory was empty; it now holds `founder.webp` and `why.webp`, so the file was doing nothing.
- **Re-audited mechanically and still clean:** every class in every CSS Module is referenced from JSX or `composes`d (comments stripped before matching — several deleted names, `.button-quiet`, `.button-reveal` and `.zoom-layer`, survive only in comments explaining their removal, which is correct); no unused imports; no orphaned assets; no `three`/`ogl`/`tailwind`/`FluidLens`/`Grainient` references anywhere in `src`, `index.html` or the config.
- **The token list below still holds at thirteen**, with one swap: `--text-xl` has come off it (WhyAgb's `.description` uses it) and `--duration-slow` has gone back on.
- **Two things noted and NOT touched, listed here per this section's rule:**
  - **`.noise-overlay` in `global.css` has no call site.** It is the per-surface form of the global film grain and is documented as a utility in §2; ~15 lines, zero runtime cost while unapplied. Kept as the way a future raised surface gets grain, not as an oversight.
  - **`LogoLoop`'s `fadeOut` / `fadeOutColor` props are unused by both call sites**, along with the `[data-fade='true']` edge-mask rules in its stylesheet (~30 lines). `LogoLoop` is a generic strip component with a documented prop surface, and the Founder's two strips deliberately run to the section's edges; this is API breadth, not dead code from a removed approach.
- **Flagged, not acted on — the one real bloat item is `public/assets/images/agb-logo.png`: 195 KB, 360×672.** It is fetched by *every* visitor as the favicon (`index.html` points `rel="icon"` at the full-size file, so a 16px tab icon costs 195 KB) and drawn three more times at ≤6.5rem tall. A WebP at the same pixel dimensions would land around 15–25 KB. **This is not a change to make unasked** — §5 rule 5 keeps the logo as-is, and re-encoding is close enough to that line to need the user's call. It is also the largest asset on the site that is *not* footage.
...
**Eleven design tokens in `variables.css` are unreferenced, and are deliberately kept:** `--color-gold-deep`, `--color-border`, `--color-border-subtle`, `--color-overlay`, `--glass-sheen`, `--glass-sheen-warm`, `--header-blur`, `--text-lg`, `--duration-slow`, `--tracking-widest`, `--radius-sm`. **`--ease-in-out` has come off this list** — both headers' nav-link hover use it, the first call sites it has ever had, which is exactly what it was kept for. It briefly had a third, the Team section's circle hover, removed 2026-08-01 along with the rest of that hover (see below). `--duration-slow` is unreferenced again for the same reason it was before that hover existed: 600ms is the right feel for something settling and the wrong one for a pointer state. **`--color-glass` and `--text-2xl` have also come off it, both on 2026-08-01**: the Founder's metadata cards are the "future raised surface" `--color-glass` was kept for (see the redesign note below), and WhyAgb's heading came down a step in size and landed on `--text-2xl`, the same edit that took its weight down from the h2 default. They are listed here rather than deleted, per this section's own rule. Most are **rungs of a scale** — deleting the unused steps of a type, radius or easing ramp leaves a ramp with holes in it, which is worse than an unused custom property that costs nothing at runtime. The palette entries are the same argument: they are what a new surface is built from. `--glass-sheen` is the newest arrival, orphaned when the hero ticker moved to `FluidBar`; `--header-blur` was orphaned when the header pill went to white glass.
...
### The Founder's redesign, and the correction pass the same day — 2026-08-01

The section changed twice in one day. The first pass (a redesign against a reference image) shipped a portrait halo, card-style facts, a five-node career timeline and a bordered photo strip. A second, same-day pass corrected real problems the first one left behind — a genuine visual bug, not just taste — and **this entry describes the corrected, current state**; nothing below should be read as the first pass's account, which it has replaced.

**The halo is gone, not merely retuned — it was the section's actual overflow bug.** It sized itself off `.portrait-frame`'s own *width* with `aspect-ratio: 1`, and the frame is wide and short (a rectangle, not the square that assumed); on top of a width-derived circle already clearing the frame's top and bottom edges by a wide margin, its blurred `box-shadow` glow added more still. With only `.founder`'s own small block-start padding above it, that glow reached past the section's own top edge into the Story section rendered before it — literal visual bleed into a neighbouring section, not a metaphor. `.portrait-wrap` and `.portrait-halo` are both deleted; the portrait is a plain photograph again, exactly the box `.portrait-frame` always clipped, no ring, no glow, nothing that can paint outside its own box.

**The intro column regrouped**, and this is a genuine structural change, not a revert. It used to be two columns — the name paired with the portrait (`.identity`), the eyebrow/bio/facts paired together (`.profile`) — which put the eyebrow and the name, one label and the thing it labels, in different tracks of the grid. There is no more `.identity`: `.profile` now carries all four text pieces in reading order (eyebrow, name, bio, facts) as one column, and `.portrait-frame` is `.intro`'s other, plain grid child, on the right. The 48rem grid split flipped from `2fr 3fr` to `3fr 2fr` to keep the same copy-to-photograph ratio now that copy is first. The `.profile` `padding-block-start` that used to line the eyebrow up with the portrait's top edge is gone — nothing needs cross-column alignment once the eyebrow is simply the first line of its own column.

**`--founder-shift` is deleted outright**, not retuned. It was a compensated top-padding scheme that held the intro block visually still while the career block (which sat second, directly after it) and the strip (at half the amount) dropped away, on the argument that `justify-content: center` redistributes added height as an equal push up/down from the column's own middle. Two things broke it at once: the first redesign pass's padded cards, floating-icon timeline and bordered strips pushed the column's natural height past 100vh on ordinary desktop windows, not just on a phone — the one case the scheme's own comment had already priced in — which left `justify-content: center` with no free space to redistribute and the extra padding as pure dead weight, the section's single largest source of excess height. And the reorder below broke the scheme's other premise, that career sits second. Rebuilding an equivalent for a new, reordered, no-longer-overflowing section was not worth it; `.founder`, `.strip` and `.career` each carry a flat margin/padding clamp now, no shared token between them.

**Career Highlights moved to close the section**, below both scrolling strips rather than directly after the intro — the order is now intro, photo strip, title strip, timeline. Every node's icon now renders *inside* its circle, the first node included — there is no longer a floating-icon variant, and no clearance padding reserved above the row for one. `--timeline-node-size` is the new, smaller circle every node but the first draws its icon inside; `--timeline-rail` is still the fixed square every node sits centred in, first or not, which is what keeps all five on one line. **The connecting line no longer depends on there being exactly five items.** Each `.timeline-item` draws its own full-length segment (`::before`, 0% to 100% of its own box) that joins edge-to-edge with its neighbours' segments because items sit with zero gap; `:first-child`/`:last-child` trim their own segment to start/end at their own 50% rather than their outer edge, so the rail begins exactly at the first node and ends exactly at the last one without any percentage tied to the item count. **The line is continuous at every width, including on a phone** — it no longer drops there. The same per-item-segment technique just runs on the other axis: the list becomes a column, each item becomes a row (rail left, copy right, the rail vertically centred against the copy by `align-items: center` rather than top-aligned), and the trims become `inset-block-start`/`inset-block-end` instead of inline ones. Neither axis needs to know the other items' heights or widths — only its own item's own box — which is what makes the same trick work for text of any length.

**The photo strip lost its "Selected Works" label**, and nothing else about it changed — still a bordered card (`--glass-fill`, `--glass-border`, `--radius-lg`), still clipped by `.loop { overflow: hidden }` rather than a fade mask, still on the `container-max` column. The arrow-nav buttons a reference design once showed still do not exist anywhere in this codebase's LogoLoop and still were never added.
...
### The second stage: Team arrives over a pinned WhyAgb — 2026-08-01

**The same technique as above, applied a second time, and it is deliberately the same technique.** `.closing-stage` in `pages/HomePage.module.css` holds exactly `<WhyAgb />` and `<Team />`; `WhyAgb` is `position: sticky` inside it; Team scrolls up over it on its own rounded top edge and eventually covers it outright. The wrapper's bottom edge ends the pin as a fact of layout, so — as with `.stage` — **there is no threshold for anything to compute.**

**It is not the scroll-zoom coming back, and the difference is the whole point.** That one (removed 2026-07-31, above) was `position: sticky` *plus* `useScroll`/`useTransform` writing a scale, three counter-scales, an elliptical radius and a blur into `will-change` layers every frame. This is one declaration: `position: sticky; inset-block-start: 0`. No listener, no observer, no transform, no `will-change`, nothing recomputed per frame. The compositor moves a box it already holds. **Sticky was never what was wrong with the zoom** — the per-frame JS was — and copying `HeroBackdrop`'s shape rather than inventing one is what keeps that distinction visible in the code.

**The arithmetic.** Wrapper height `H = W + T` for WhyAgb's height `W` and Team's `T`; WhyAgb's own top is `clamp(s, 0, H - W)`, so the travel is exactly `T`.

- `0 < s < W` — WhyAgb pinned and motionless, Team's top edge at `W - s`, the curve crossing the full height of the screen.
- `s = W` — Team covers the viewport completely. `W` is one viewport wherever the pin is on and `T` is at least one, so the pin is still live with travel to spare. This is the moment the mechanic exists for.
- `s = T` — WhyAgb unpins and scrolls away *behind* a Team that already fills the screen, so none of it is visible while it goes.

**How it differs from `.stage`, in one respect only.** Up there the sticky element is a *backdrop* — not a section — so the screen of flow height it claims has to be handed back by `.stage-content`'s `-100vh`. Here the sticky element **is** a section and that screen is legitimately the page's, so **nothing is given back, there is no inner wrapper, and the document is exactly as tall as it was before the pairing existed.** The `-100vh` trap documented above does not arise; do not add a margin here looking for it.

**`.closing-stage` is bare for the same reasons `.stage` is**, plus one of its own: **no `background`**, because the notches Team's rounded corners leave have to show WhyAgb behind them. No `overflow` (it would capture the sticky child), no `transform`/`filter`/`perspective`/`contain` (each re-scopes it), no `position` and no `z-index`.

**Team paints over WhyAgb by document order, not by z-index.** Both sit at `--z-base` in the root stacking context — which is exactly why the wrapper must stay non-stacking — and the later of two positioned siblings at the same level wins. Raising Team's z-index would work today and would be the wrong fix: it would state as a decision what is a consequence, and it would lift Team over a Founder that is at `--z-base` for unrelated reasons.

**THE PIN IS GATED TO `min-width: 64rem` AND `min-height: 45rem`, and the gate is structural rather than cosmetic.** Sticky cannot lift a box's own top edge above its offset, so a pinned section taller than the viewport does not merely scroll oddly — everything past its first screen is **unreachable for the life of the page**. `64rem` (1024px) is **not** the `48rem` where WhyAgb's grid becomes 2 × 3, and the difference was measured: at 768px the columns are ~337px, the longer titles wrap to two lines, and the section works out around 810px — which would strand its last row on a 720px window, the exact failure the gate exists to prevent, *inside* the gate. By 1024px the columns are ~465px and the section comes in around 690px. `45rem` is the 720px of the 1280×720 window its one-screen budget was measured at. **1024 × 720 is the tight corner, with roughly 30px to spare — anything that makes WhyAgb taller must be re-measured there, and raising the gate is the right response rather than shrinking the section.** Outside the envelope the two are ordinary siblings in flow, exactly as before.

**Team's rounded block-start corners are part of the mechanic, and the radius is small on purpose.** `--team-radius` is `clamp(0.5rem, 1.5vw, 1.25rem)`. It was four times that, sized on the argument that the curve must stay legible once the section fills the window — which was the wrong problem: **at full cover the curve is meant to be gone, not legible** (see the tuck, below), which frees the radius to be sized for the only moment it is seen, mid-slide against WhyAgb.

**`overflow: hidden` on the section is necessary and is NOT sufficient, and this is the trap.** A `<video>` gets its own compositing layer, and so does anything carrying a `backdrop-filter`. An ancestor's `border-radius` + `overflow: hidden` is a paint-time clip on the *ancestor's* layer and does not reliably reach a separately composited descendant in Chromium or WebKit. The symptom is specific and was seen: the pane rendered correctly rounded while the footage under it kept square corners, and that square corner showing past the curve read as a hard edge across the top of the section. **So the radius is declared on each layer that has one — the section, `.backdrop` (the video) and `.glass` (the pane) — three copies of `--team-radius`, which is why it is a custom property. Any new full-bleed layer at this section's block-start edge needs the same two lines.**

**NEVER PUT A BORDER ON THAT EDGE.** A 1px `--glass-border` hairline was added there to make the curve legible and was the single worst thing that could have gone there: `overflow` clips descendants to the **padding** box, which a border pushes inward, so the video was clipped 1px *inside* the section's own rounded edge — and 1px of white-at-0.08 over opaque black composites to about `rgb(20,20,19)`, tracing the whole top edge and both corners in a near-black line against whatever bright frame was underneath. **That was the "black hard edge".** If the curve ever needs marking, use an inset shadow or a pseudo-element; neither moves the clip.

**`--team-tuck` is what makes the corner disappear at full cover, and it is height rather than a conditional radius.** Team's top edge reaches the viewport top at `s = W`, and if the section were exactly one screen tall that is also the end of the document — the curve would be parked on the viewport's top edge permanently, which is a notch cut out of the page rather than an edge. The tuck (`--team-radius + 0.25rem`) is added to the section's `min-block-size`, giving the document that much more scroll so Team keeps rising after it has covered the screen and the curve travels off the top under its own steam. **No observer, no threshold, no toggled class** — the corner is visible exactly while there is something behind it to be an edge against.

Four things carry the tuck and **all four must move together**: `min-block-size`, `padding-block-start` (adding it to the height alone would grow the box around a centred column and drop every element in the section by `tuck / 2`; adding it to the padding as well restores the content box to its original height and puts the content exactly where it was on screen at full cover), and the `block-size` of **both** `.backdrop` and `.glass` — a ground left at `100vh` leaves a hard black band of bare `--color-black` along the bottom `tuck` pixels of the viewport. It is `0px` outside the pin's envelope, set by a media query in `Team.module.css` that is **a copy of WhyAgb's two conditions and must stay identical** — one decision in two files, because it applies to two elements in two components and a custom property cannot cross that boundary.

**The `overflow` is safe on Team specifically because Team is the sticky element's *sibling*, not its ancestor** — `overflow` on the wrong element is the one thing that silently breaks a sticky pairing. It clips nothing it should not: the footer's negative margins reach exactly the border box, and `.button-featured`'s glow ring is already clipped by its own variant.

**The one cost, stated rather than hidden:** WhyAgb's `backdrop-filter` and Team's are both live for the full screen of scrolling in which Team rises, where before they overlapped only briefly at a section boundary. Both are the cheap light variant (7px, not 20). Switching WhyAgb's off once it is covered means an observer and a threshold — precisely the machinery `HeroBackdrop`'s rewrite deleted — and is not worth buying back for this.
...
| `.button-featured::after`'s rim light | `data-offscreen` on **`.hero` or the Team `<section>`** — one per call site | Animates a registered custom property into a conic-gradient behind two masks composited with `xor`: every frame is a full re-raster, not a compositor transform. The variant has two call sites now; each is gated by its own ancestor, which is why the second needed no stylesheet change. |
| `LogoLoop`'s track | **deleted — nothing to gate** | Both of Founder's strips were removed 2026-08-02, and `LogoLoop` went with them as their only caller — see §7. |
...
| WhyAgb's three wordmark tracks | **deleted — nothing to gate** | They were left running on the same argument as the sheens: `transform` only, no filtered ancestor, no `will-change`. The whole field is gone now, so the closing section has no infinite animation at all. |
...
- **Three viewport-wide `backdrop-filter`s**, `.about`'s, `.founder`'s `.glass` and now `.why`'s, and together they are the most expensive thing in the project. All three stay — the glass *is* those sections' surface (§2). `--section-glass-blur` is the dial, not the fill. No two are ever fully on screen together, but each adjacent pair overlaps at its section boundary, so for part of one scroll two are live. **The two are not equally necessary:** About's has to reach outside itself, through the document to the hero's *fixed* footage, and `backdrop-filter` is the only mechanism that can; Founder's only has a sibling `<img>` behind it, so it could be `filter: blur()` on that image plus a flat fill here for less, at the same visual result. It is written to match About because one glass idiom is worth more than the saving until the saving is measured. That is the first thing to change if the Founder ever feels heavy. **WhyAgb's is the same case as the Founder's and exists for the same two reasons** — it holds the copy off a photograph with a lit wordmark through the middle of it, and it makes its side of the Founder boundary the same material as the other side. It is the light pair (7px, not 20), so it is the cheap variant; but swapping either of the two closing panes for `filter: blur()` on its own image means inflating that image past its box, which changes its rendered width — and equal rendered width is what the seam between them depends on. Change them together or not at all.
